...
short_description: Kubernetes DaemonSetList
description:
- Retrieve a list of daemon_sets. List operations provide a snapshot read of the underlying
  ...
  contains:
    ...
    items:
      ...
      contains:
        ...
        status:
          description:
          - Status is the current status of this daemon set. This data may be out
            of date by some window of time. Populated by the system. Read-only.
          type: complex
          contains:
            current_number_scheduled:
              description:
              - CurrentNumberScheduled is the number of nodes that are running at
                least 1 daemon pod and are supposed to run the daemon pod.
              type: int
            desired_number_scheduled:
              description:
              - DesiredNumberScheduled is the total number of nodes that should be
                running the daemon pod (including nodes correctly running the daemon
                pod).
              type: int
            number_misscheduled:
              description:
              - NumberMisscheduled is the number of nodes that are running the daemon
