...
daemon_set:
  ...
  contains:
      ...
      contains:
        selector:
          ...
          - Selector is a label query over pods that are managed by the daemon set.
            Must match in order to be controlled. If empty, defaulted to labels on
            Pod template.
          type: complex
          contains:
            match_expressions:
        ...
        template:
          description:
          - Template is the object that describes the pod that will be created. The
            DaemonSet will create exactly one copy of this pod on every node that
            matches the template's node selector (or on every node if no node selector
            is specified).
          type: complex
          contains:
            metadata:
    ...
    status:
      description:
      - Status is the current status of this daemon set. This data may be out of date
        by some window of time. Populated by the system. Read-only.
      type: complex
      contains:
        current_number_scheduled:
          description:
          - CurrentNumberScheduled is the number of nodes that are running at least
            1 daemon pod and are supposed to run the daemon pod.
          type: int
